...
def remove_deps(df):
    df = df.drop_duplicates(subset=['sample_id', 'tools', 'data_type', 'bacteria_finalname'])
    df = df.drop(columns=['relative_abundance'])
    df.to_csv('results_removeduplicate.csv', index=False)
    return df
...
# Save the results
save_results(classification_results, 'classification_results')