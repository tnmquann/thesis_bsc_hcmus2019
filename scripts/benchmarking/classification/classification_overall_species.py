...
def remove_deps(df):
    df = df.drop_duplicates(subset=['tools', 'data_type', 'bacteria_finalname'])
    df = df.drop(columns=['relative_abundance', 'sample_id', 'genus', 'bacteria'])
    df.to_csv('results_removeduplicate_overall.csv', index=False)
    return df
...
reference_cell_count = reference_cell['bacteria_finalname'].nunique()
reference_dna_count = reference_dna['bacteria_finalname'].nunique()
reference_mixed_count = reference_mixed['bacteria_finalname'].nunique()

def classification_overall(df, reference_df, reference_count):
    df_grouped = df.groupby(['tools', 'data_type', 'tools_detail'])
    classification_results = []
    for name, group in df_grouped:
        TP = group['bacteria_finalname'].isin(reference_df['bacteria_finalname']).sum()
        FP = len(group) - TP
...
# Save the results
classification_results.to_csv('classification_results_overall.csv', index=False)
classification_results.to_excel('classification_results_overall.xlsx', index=False)